...
def parse_yx_pos_from_name(n):
    ...
    m = re.search(r'Tile_(\d+)-(\d+)_s\d+', basename)
    if not m:
        raise ValueError(f"Could not parse tile position from filename: {basename}")

    x, y = map(int, m.groups())

    # Convert from 1-indexed x/y in filename to 0-indexed (y, x)
    return (y - 1, x - 1)
